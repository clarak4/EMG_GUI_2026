...
class CircularGauge(QWidget):
    ...
    def __init__(self,
        ...
        self.ratio = 0.5  # 0.0 = full compensation, 1.0 = full target
    ...
    def drawOuterArc2(self, painter: QPainter, radius: float):
        """Draw the dynamic green/orange arc based on the live ratio."""
        arc_rect = QRectF(-radius, -radius, 2 * radius, 2 * radius)
        painter.save()

        # Normalize angles
        start_angle = self.normalize_angle(self.start_angle)
        span = self.angle_range  # Note: may be negative

        # Target muscle: green arc
        green_span = self.ratio * span
        painter.setPen(QPen(QColor("#009E73"), self.outer_circle_thickness // 2))  # Green
        painter.drawArc(arc_rect, int(-start_angle * 16), int(-green_span * 16))

        # Compensation muscle: orange arc
        orange_start = start_angle + green_span
        orange_span = span - green_span
        painter.setPen(QPen(QColor("#E69F00"), self.outer_circle_thickness // 2))  # Orange
        painter.drawArc(arc_rect, int(-orange_start * 16), int(-orange_span * 16))
    ...
    def drawNumbers(self, painter: QPainter, radius: float):
        ...
        # pen = QPen(Qt.darkGray, 2)  # to set the numbers color
        # pen = QPen(QColor(137, 148, 153), 2)  # to set the numbers color
        pen = QPen(QColor("#D8FFFD"), 2)  # to set the numbers color
        painter.setPen(pen)
        angle_step = self.angle_range / self.steps  # Degrees between ticks
    ...
    def drawNeedle(self, painter: QPainter, radius: float):
        ...
        painter.save()
        needle_angle = self.start_angle + self.ratio * self.angle_range
        ...
        painter.rotate(needle_angle)
        # pen = QPen(Qt.red, 1)
        pen = QPen(QColor("#D8FFFD"), 1)
        painter.setPen(pen)
        painter.setBrush(QColor("#D8FFFD"))
        painter.drawPolygon(needle)
        painter.restore()
...
class Windows(QMainWindow):
    def __init__(self) -> None:
        ...
        self.slider.valueChanged.connect(lambda value: self.updateRatio(value / 30))  # 0.0–1.0
    ...
    def updateRatio(self, ratio: float):
        self.gauge.setRatio(ratio)
